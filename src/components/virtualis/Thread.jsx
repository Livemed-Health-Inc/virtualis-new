...
import { T, mono, ACUITY, inputStyle, Avatar, Glyph, AcuityBadge, Back, PersonIcon, DoorIcon, VideoIcon, FacilityChip } from "./ui";
...
export default function Thread({ t, onBack, onDetail, onVideo, embedded }) {
  ...
  useEffect(() => { endRef.current?.scrollIntoView({ behavior: "smooth" }); }, [extra.length, t.id]);
  useEffect(() => { setExtra([]); setAckd(false); setSheet(null); }, [t.id]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", minHeight: 0, animation: embedded ? "fadeIn .2s ease" : "slideIn .32s cubic-bezier(.2,.8,.3,1)", position: "relative", background: T.bg }}>
      <div style={{ background: "rgba(255,255,255,.86)", backdropFilter: "blur(18px)", borderBottom: "1px solid " + T.line, padding: "12px 14px 10px", position: "sticky", top: 0, zIndex: 10 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 2 }}>
              ...
              <span style={{ fontSize: 11.5, color: T.sub, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{t.team ? t.members : t.context}</span>
            </div>
          </div>
          <button onClick={onVideo} title="Start telehealth visit" style={{ all: "unset", cursor: "pointer", background: "linear-gradient(135deg,#2E5CFF,#1E3FCC)", color: "#fff", fontSize: 12.5, fontWeight: 640, borderRadius: 20, padding: "9px 14px", display: "flex", alignItems: "center", gap: 6, flexShrink: 0, boxShadow: "0 6px 16px rgba(41,112,255,.28)" }}>
            <VideoIcon /> Video
          </button>
          <button title="Voice call" style={{ all: "unset", cursor: "pointer", background: "#101828", color: "#fff", borderRadius: 20, padding: "10px 12px", display: "flex", alignItems: "center", flexShrink: 0 }}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 4h4l2 5-2.5 1.5a11 11 0 0 0 5 5L15 13l5 2v4a2 2 0 0 1-2 2A16 16 0 0 1 3 6a2 2 0 0 1 2-2Z" /></svg>
          </button>
        </div>
        {t.patient !== "—" && (
          <button onClick={onDetail} style={{ all: "unset", boxSizing: "border-box", cursor: "pointer", marginTop: 10, width: "100%", display: "flex", alignItems: "center", gap: 8, background: T.blueSoft, border: "1px solid #D6E4FF", borderRadius: 13, padding: "9px 13px", flexWrap: "wrap" }}>
        ...
        {t.acuity === "critical" && (
          <div style={{ display: "flex", alignItems: "center", gap: 9, marginTop: 8, background: ackd ? "#EDFBF3" : "#FEF0EF", border: "1px solid " + (ackd ? "#C9F0DB" : "#FBD9D6"), borderRadius: 13, padding: "8px 12px" }}>
            <Glyph level="critical" size={8} gap={1.5} w={3.5} pulse={!ackd} />
            <span style={{ fontSize: 12.5, fontWeight: 600, color: ackd ? T.green : T.red, flex: 1 }}>{ackd ? "Acknowledged — escalation cleared" : "Critical consult · acknowledge to stop escalation"}</span>
            {!ackd && <button onClick={() => setAckd(true)} style={{ all: "unset", cursor: "pointer", fontSize: 12.5, fontWeight: 650, color: "#fff", background: T.red, borderRadius: 14, padding: "6px 13px" }}>Acknowledge</button>}
          </div>
        )}
      </div>

      <div style={{ flex: 1, overflowY: "auto", padding: "12px 14px", display: "flex", flexDirection: "column", gap: 11 }}>
        <div style={{ width: "100%", maxWidth: 820, margin: "0 auto", display: "flex", flexDirection: "column", gap: 11 }}>
          {msgs.map((m, i) => (
            <div key={i} style={{ alignSelf: m.me ? "flex-end" : "flex-start", maxWidth: "min(82%, 560px)" }}>
                  ...
                  <div style={{ background: "#0A0F1E", border: "1px solid #1C2740", borderRadius: 18, padding: "13px 16px", boxShadow: `0 10px 26px rgba(10,15,30,.35), 0 0 0 1.5px ${ACUITY[t.acuity].color}33` }}>
                    ...
                    <div style={{ display: "flex", alignItems: "flex-end", gap: 2.5, marginTop: 11, height: 22 }}>
                      {[6, 12, 8, 16, 10, 20, 14, 8, 12, 18, 9, 15, 7, 11, 17, 10, 6, 13, 8].map((h, j) => (
                        <span key={j} style={{ width: 3, height: h, borderRadius: 2, background: "#2E5CFF", opacity: 0.9, animation: `eq 1.2s ${j * 0.05}s ease-in-out infinite alternate` }} />
                      ))}
                    </div>
              ...
              {sheet === i && (
                <div style={{ background: "#fff", border: "1px solid " + T.line, borderRadius: 16, marginTop: 6, boxShadow: "0 14px 34px rgba(16,24,40,.14)", overflow: "hidden", animation: "rise .2s ease" }}>
                  <div style={{ display: "flex", gap: 6, padding: "10px 12px", borderBottom: "1px solid " + T.line, flexWrap: "wrap" }}>
                    {[["✓", "Acknowledge"], ["👍", "Agree"], ["❗", "Escalate"]].map(([e, l]) => (
                      <button key={l} onClick={() => { setSheet(null); if (l === "Acknowledge") setAckd(true); }} style={{ all: "unset", cursor: "pointer", fontSize: 12, fontWeight: 620, color: T.ink, background: "#F7F8FA", border: "1px solid " + T.line, borderRadius: 16, padding: "6px 12px" }}>{e} {l}</button>
                    ))}
                  </div>
      ...
      <div style={{ padding: "8px 14px 12px" }}>
        <div style={{ maxWidth: 820, margin: "0 auto", display: "flex", gap: 8, alignItems: "center" }}>
          <button title="Attach" style={{ all: "unset", cursor: "pointer", width: 42, height: 42, borderRadius: 14, background: "#EDF0F4", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
          ...
          <input value={draft} onChange={(e) => setDraft(e.target.value)} onKeyDown={(e) => e.key === "Enter" && send()} placeholder="Message" style={{ ...inputStyle, borderRadius: 24, flex: 1, minWidth: 0 }} />
